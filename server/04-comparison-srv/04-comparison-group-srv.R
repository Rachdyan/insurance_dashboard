...
## EXPORT
output$comparison_export_group_ui <- renderUI({
    hidden(span(id= "comparison_group_graph_head",
        downloadBttn(
            outputId = "export_comparison_group_excel",
            "Export to Excel",
            style = 'simple',
            size = 'sm',
            icon = icon("file-excel")),
        downloadBttn(
            outputId = "export_comparison_group_pdf",
            "Export to PDF",
            style = 'simple',
            size = 'sm',
            icon = icon("file-pdf"))
...
observeEvent(input$comparison_generate_group_stat_btn, {
    show('comparison_group_graph_head')
    show('groupyearrange')
    show('comparison_group_export_div')
})
...
output$comparison_premium_written_group_ui <- renderUI({
    ...
    fluidRow(
        ...
        br(),
        plotlyOutput("comparison_written_premium_group_plot", width = '85%', height = "475px") %>% withSpinner(),
        align = "center"
    )
})
...
comparison_group_written_premium_export_table <- reactive({
    if(nrow(comparison_group_written_premium_data()) == 0){
        zeroGrob()
    } else {
        comparison_group_written_premium_data() %>%
            group_by(groupname) %>%
...
comparison_group_written_premium_export_table_excel <- reactive({
    if(nrow(comparison_group_written_premium_data()) == 0){
        tibble()
    } else {
        comparison_group_written_premium_data() %>%
...
comparison_group_written_premium_ggplot_export <- reactive({
    comparison_group_written_premium_ggplot() +
...
output$comparison_yoy_change_premium_written_group_ui <- renderUI({
    ...
    fluidRow(
        ...
        br(),
        plotlyOutput("comparison_group_yoy_change_written_premiums_plot", width = '85%', height = "475px") %>% withSpinner(),
        align = "center"
    )
})
...
comparison_group_yoy_change_written_premium_export_table <- reactive({
    if(nrow(comparison_group_yoy_change_written_premiums_data()) == 0){
        zeroGrob()
    } else {
        comparison_group_yoy_change_written_premiums_data() %>%
            group_by(groupname) %>%
...
comparison_group_yoy_change_written_premium_export_table_excel <- reactive({
    if(nrow(comparison_group_yoy_change_written_premiums_data()) == 0){
        tibble()
    } else {
        comparison_group_yoy_change_written_premiums_data() %>%
...
comparison_group_yoy_change_written_premiums_ggplot_export <- reactive({
    comparison_group_yoy_change_written_premiums_ggplot() +
...
output$comparison_premium_earned_group_ui <- renderUI({
    ...
    fluidRow(
        ...
        br(),
        plotlyOutput("comparison_earned_premium_group_plot", width = '85%', height = "475px") %>% withSpinner(),
        align = "center"
    )
})
...
comparison_group_earned_premium_export_table <- reactive({
    if(nrow(comparison_group_earned_premium_data()) == 0){
        zeroGrob()
    } else {
        comparison_group_earned_premium_data() %>%
            group_by(groupname) %>%
...
comparison_group_earned_premium_export_table_excel <- reactive({
    if(nrow(comparison_group_earned_premium_data()) == 0){
        tibble()
    } else {
        comparison_group_earned_premium_data() %>%
...
comparison_group_earned_premium_ggplot_export <- reactive({
    comparison_group_earned_premium_ggplot() +
...
output$comparison_total_losses_group_ui <- renderUI({
    ...
    fluidRow(
        ...
        br(),
        plotlyOutput("comparison_total_losses_group_plot", width = '85%', height = "475px") %>% withSpinner(),
        align = "center"
    )
})
...
comparison_group_total_losses_export_table <- reactive({
    if(nrow(comparison_group_total_losses_data()) == 0){
        zeroGrob()
    } else{
        comparison_group_total_losses_data() %>%
            group_by(groupname) %>%
...
comparison_group_total_losses_export_table_excel <- reactive({
    if(nrow(comparison_group_total_losses_data()) == 0){
        tibble()
    } else {
        comparison_group_total_losses_data() %>%
...
comparison_group_total_losses_ggplot_export <- reactive({
    comparison_group_total_losses_ggplot() +
...
output$comparison_loss_ratio_group_ui <- renderUI({
    ...
    fluidRow(
        ...
        br(),
        plotlyOutput("comparison_loss_ratio_group_plot", width = '85%', height = "475px") %>% withSpinner(),
        align = "center"
    )
})
...
comparison_group_loss_ratio_export_table <- reactive({
    if(nrow(comparison_group_loss_ratio_data()) == 0){
        zeroGrob()
    } else{
        comparison_group_loss_ratio_data() %>%
            group_by(groupname) %>%
...
comparison_group_loss_ratio_export_table_excel <- reactive({
    if(nrow(comparison_group_loss_ratio_data()) == 0){
        tibble()
    } else {
        comparison_group_loss_ratio_data() %>%
...
comparison_group_loss_ratio_ggplot_export <- reactive({
    comparison_group_loss_ratio_ggplot() +
        ggtitle("Loss Ratio") +
        single_graph_export_theme
})
...
output$comparison_num_of_policies_group_ui <- renderUI({
    ...
    fluidRow(
        ...
        br(),
        plotlyOutput("comparison_num_of_policies_group_plot", width = '85%', height = "475px") %>% withSpinner(),
        align = "center"
    )
})
...
comparison_group_num_of_policies_export_table <- reactive({
    if(nrow(comparison_group_num_of_policies_data()) == 0){
        zeroGrob()
    } else {
        comparison_group_num_of_policies_data() %>%
            group_by(groupname) %>%
...
comparison_group_num_of_policies_export_table_excel <- reactive({
    if(nrow(comparison_group_num_of_policies_data()) == 0){
        tibble()
    } else {
        comparison_group_num_of_policies_data() %>%
...
comparison_group_num_of_policies_ggplot_export <- reactive({
    comparison_group_num_of_policies_ggplot() +
...
output$comparison_avg_premium_group_ui <- renderUI({
    ...
    fluidRow(
        ...
        br(),
        plotlyOutput("comparison_avg_premium_group_plot", width = '85%', height = "475px") %>% withSpinner(),
        align = "center"
    )
})
...
comparison_group_avg_premium_export_table <- reactive({
    if(nrow(comparison_group_avg_premium_data()) == 0){
        zeroGrob()
    } else {
        comparison_group_avg_premium_data() %>%
            group_by(groupname) %>%
...
comparison_group_avg_premium_export_table_excel <- reactive({
    if(nrow(comparison_group_avg_premium_data()) == 0){
        tibble()
    } else {
        comparison_group_avg_premium_data() %>%
...
comparison_group_avg_premium_ggplot <- eventReactive(c(input$comparison_generate_group_stat_btn, input$group_year_range), {
    if(nrow(comparison_group_avg_premium_data()) == 0){
        ...
        ggplot(comparison_group_avg_premium_data(), aes(x = as.factor(year), y = avg_premium, group = groupname, color = groupname, text = glue("<span><b>Value:</b> ${comma(avg_premium)}"))) +
            ...
            scale_fill_manual(values = comparison_group_color_palette()) +
            geom_point(aes(fill = groupname), shape=21, size=4, color = 'black') +
            scale_y_continuous(limits = c(-500, max(comparison_group_avg_premium_data()$avg_premium, na.rm = T) *1.3 ), labels = label_number(prefix = "  $", scale_cut = cut_short_scale()), expand = c(0.001,0)) +
            ylab("Average Premium") +
            universal_graph_theme_with_legend(0.125, 0.9)
    }
})

comparison_group_avg_premium_ggplot_export <- reactive({
    comparison_group_avg_premium_ggplot() +
...
output$comparison_direct_defense_group_ui <- renderUI({
    ...
    fluidRow(
        ...
        br(),
        plotlyOutput("comparison_direct_defense_group_plot", width = '85%', height = "475px") %>% withSpinner(),
        align = "center"
    )
})
...
comparison_group_direct_defense_export_table <- reactive({
    if(nrow(comparison_group_direct_defense_data()) == 0){
        zeroGrob()
    } else {
        comparison_group_direct_defense_data() %>%
            group_by(groupname) %>%
...
comparison_group_direct_defense_export_table_excel <- reactive({
    if(nrow(comparison_group_direct_defense_data()) == 0){
        tibble()
    } else {
        comparison_group_direct_defense_data() %>%
...
comparison_group_direct_defense_ggplot_export <- reactive({
    comparison_group_direct_defense_ggplot() +
...
output$comparison_case_reserves_group_ui <- renderUI({
    ...
    fluidRow(
        ...
        br(),
        plotlyOutput("comparison_case_reserves_group_plot", width = '85%', height = "475px") %>% withSpinner(),
        align = "center"
    )
})
...
comparison_group_case_reserves_export_table <- reactive({
    if(nrow(comparison_group_case_reserves_data()) == 0){
        zeroGrob()
    } else {
        comparison_group_case_reserves_data() %>%
            group_by(groupname) %>%
...
comparison_group_case_reserves_export_table_excel <- reactive({
    if(nrow(comparison_group_case_reserves_data()) == 0){
        tibble()
    } else {
        comparison_group_case_reserves_data() %>%
...
comparison_group_case_reserves_ggplot_export <- reactive({
    comparison_group_case_reserves_ggplot() +
...
output$comparison_total_claims_closed_with_payment_group_ui <- renderUI({
    ...
    fluidRow(
        ...
        br(),
        plotlyOutput("comparison_total_claims_closed_with_payment_group_plot", width = '85%', height = "475px") %>% withSpinner(),
        align = "center"
    )
})
...
comparison_group_total_claims_closed_with_payment_export_table <- reactive({
    if(nrow(comparison_group_total_claims_closed_with_payment_data()) == 0){
        zeroGrob()
    } else {
        comparison_group_total_claims_closed_with_payment_data() %>%
            group_by(groupname) %>%
...
comparison_group_total_claims_closed_with_payment_export_table_excel <- reactive({
    if(nrow(comparison_group_total_claims_closed_with_payment_data()) == 0){
        tibble()
    } else {
        comparison_group_total_claims_closed_with_payment_data() %>%
...
comparison_group_total_claims_closed_with_payment_ggplot_export <- reactive({
    comparison_group_total_claims_closed_with_payment_ggplot() +
...
output$comparison_total_claims_closed_without_payment_group_ui <- renderUI({
    ...
    fluidRow(
        ...
        br(),
        plotlyOutput("comparison_total_claims_closed_without_payment_group_plot", width = '85%', height = "475px") %>% withSpinner(),
        align = "center"
    )
})
...
comparison_group_total_claims_closed_without_payment_export_table <- reactive({
    if(nrow(comparison_group_total_claims_closed_without_payment_data()) == 0){
        zeroGrob()
    } else {
        comparison_group_total_claims_closed_without_payment_data() %>%
            group_by(groupname) %>%
...
comparison_group_total_claims_closed_without_payment_export_table_excel <- reactive({
    if(nrow(comparison_group_total_claims_closed_without_payment_data()) == 0){
        tibble()
    } else {
        comparison_group_total_claims_closed_without_payment_data() %>%
...
comparison_group_total_claims_closed_without_payment_ggplot_export <- reactive({
    comparison_group_total_claims_closed_without_payment_ggplot() +
...
output$comparison_first_party_claims_closed_without_payment_group_ui <- renderUI({
    ...
    fluidRow(
        ...
        br(),
        plotlyOutput("comparison_first_party_claims_closed_without_payment_group_plot", width = '85%', height = "475px") %>% withSpinner(),
        align = "center"
    )
})
...
comparison_group_first_party_claims_closed_without_payment_export_table <- reactive({
    if(nrow(comparison_group_first_party_claims_closed_without_payment_data()) == 0){
        zeroGrob()
    } else {
        comparison_group_first_party_claims_closed_without_payment_data() %>%
            group_by(groupname) %>%
...
comparison_group_first_party_claims_closed_without_payment_export_table_excel <- reactive({
    if(nrow(comparison_group_first_party_claims_closed_without_payment_data()) == 0){
        tibble()
    } else {
        comparison_group_first_party_claims_closed_without_payment_data() %>%
...
comparison_group_first_party_claims_closed_without_payment_ggplot_export <- reactive({
    comparison_group_first_party_claims_closed_without_payment_ggplot() +
...
output$comparison_third_party_claims_closed_without_payment_group_ui <- renderUI({
    ...
    fluidRow(
        ...
        br(),
        plotlyOutput("comparison_third_party_claims_closed_without_payment_group_plot", width = '85%', height = "475px") %>% withSpinner(),
        align = "center"
    )
})
...
comparison_group_third_party_claims_closed_without_payment_export_table <- reactive({
    if(nrow(comparison_group_third_party_claims_closed_without_payment_data()) == 0){
        zeroGrob()
    } else {
        comparison_group_third_party_claims_closed_without_payment_data() %>%
            group_by(groupname) %>%
...
comparison_group_third_party_claims_closed_without_payment_export_table_excel <- reactive({
    if(nrow(comparison_group_third_party_claims_closed_without_payment_data()) == 0){
        tibble()
    } else {
        comparison_group_third_party_claims_closed_without_payment_data() %>%
...
comparison_group_third_party_claims_closed_without_payment_ggplot_export <- reactive({
    comparison_group_third_party_claims_closed_without_payment_ggplot() +
...
output$comparison_first_party_open_claims_group_ui <- renderUI({
    ...
    fluidRow(
        ...
        br(),
        plotlyOutput("comparison_first_party_open_claims_group_plot", width = '85%', height = "475px") %>% withSpinner(),
        align = "center"
    )
})
...
comparison_group_first_party_open_claims_export_table <- reactive({
    if(nrow(comparison_group_first_party_open_claims_data()) == 0){
        zeroGrob()
    } else {
        comparison_group_first_party_open_claims_data() %>%
            group_by(groupname) %>%
...
comparison_group_first_party_open_claims_export_table_excel <- reactive({
    if(nrow(comparison_group_first_party_open_claims_data()) == 0){
        tibble()
    } else {
        comparison_group_first_party_open_claims_data() %>%
...
comparison_group_first_party_open_claims_ggplot_export <- reactive({
    comparison_group_first_party_open_claims_ggplot() +
...
output$comparison_third_party_open_claims_group_ui <- renderUI({
    ...
    fluidRow(
        ...
        br(),
        plotlyOutput("comparison_third_party_open_claims_group_plot", width = '85%', height = "475px") %>% withSpinner(),
        align = "center"
    )
})
...
comparison_group_third_party_open_claims_export_table <- reactive({
    if(nrow(comparison_group_third_party_open_claims_data()) == 0){
        zeroGrob()
    } else {
        comparison_group_third_party_open_claims_data() %>%
            group_by(groupname) %>%
...
comparison_group_third_party_open_claims_export_table_excel <- reactive({
    if(nrow(comparison_group_third_party_open_claims_data()) == 0){
        tibble()
    } else {
        comparison_group_third_party_open_claims_data() %>%
...
comparison_group_third_party_open_claims_ggplot_export <- reactive({
    comparison_group_third_party_open_claims_ggplot() +
...
## PDF REPORT ---------
output$export_comparison_group_pdf = downloadHandler(
    filename = function() {glue("comparison_group.pdf")},
    content = function(file) {
        withProgress(message = glue("Exporting comparison_group.pdf"), {

            selected_group <- input$comparison_select_group %>% str_flatten_comma()
            n_selected_group <- length(input$comparison_select_group)
            ...
            cover_title3 <- textGrob(glue("Group Comparison"), gp=gpar(fontsize=12),
                x = unit(-0.15, "npc"), y = unit(0.150, "npc"), just = "left")

            cover_title4 <- textGrob(glue("{selected_group}"), gp=gpar(fontsize=10),
                x = unit(-0.15, "npc"), y = unit(0.150, "npc"), just = "left")
           ...
           ###### CONTENT PAGE
            ...
            title <- textGrob(glue("\t{selected_group}\n\n"), gp=gpar(fontsize=11, fontface = 'bold'), x = unit(0.525, "npc"))

            bottom_text <- c(1:5) %>%
                map(~ textGrob(
                    glue("Qualrisk Cyber Insurance Center | {selected_group} | {.x}"),
                    gp=gpar(fontsize=9), y = 1))

            n_plot <- length(input$comparison_select_group_statistic)
            ...
            if (!is.null(comparison_group_plots_status$premium_written_status)){
                plot_content <- append(plot_content, list(comparison_group_written_premium_ggplot_export()))
                table_content <- append(table_content, list(comparison_group_written_premium_export_table()))
            }

            if (!is.null(comparison_group_plots_status$yoy_change_premium_written_status)){
                plot_content <- append(plot_content, list(comparison_group_yoy_change_written_premiums_ggplot_export()))
                table_content <- append(table_content, list(comparison_group_yoy_change_written_premium_export_table()))
            }

            if (!is.null(comparison_group_plots_status$premium_earned_status)){
                plot_content <- append(plot_content, list(comparison_group_earned_premium_ggplot_export()))
                table_content <- append(table_content, list(comparison_group_earned_premium_export_table()))
            }

            if (!is.null(comparison_group_plots_status$total_losses_status)){
                plot_content <- append(plot_content, list(comparison_group_total_losses_ggplot_export()))
                table_content <- append(table_content, list(comparison_group_total_losses_export_table()))
            }

            if (!is.null(comparison_group_plots_status$loss_ratio_status)){
                plot_content <- append(plot_content, list(comparison_group_loss_ratio_ggplot_export()))
                table_content <- append(table_content, list(comparison_group_loss_ratio_export_table()))
            }

            if (!is.null(comparison_group_plots_status$num_of_policies_status)){
                plot_content <- append(plot_content, list(comparison_group_num_of_policies_ggplot_export()))
                table_content <- append(table_content, list(comparison_group_num_of_policies_export_table()))
            }

            if (!is.null(comparison_group_plots_status$avg_premium_status)){
                plot_content <- append(plot_content, list(comparison_group_avg_premium_ggplot_export()))
                table_content <- append(table_content, list(comparison_group_avg_premium_export_table()))
            }

            if (!is.null(comparison_group_plots_status$direct_defense_status)){
                plot_content <- append(plot_content, list(comparison_group_direct_defense_ggplot_export()))
                table_content <- append(table_content, list(comparison_group_direct_defense_export_table()))
            }

            if (!is.null(comparison_group_plots_status$case_reserves_status)){
                plot_content <- append(plot_content, list(comparison_group_case_reserves_ggplot_export()))
                table_content <- append(table_content, list(comparison_group_case_reserves_export_table()))
            }

            if (!is.null(comparison_group_plots_status$total_claims_closed_with_payment_status)){
                plot_content <- append(plot_content, list(comparison_group_total_claims_closed_with_payment_ggplot_export()))
                table_content <- append(table_content, list(comparison_group_total_claims_closed_with_payment_export_table()))
            }

            if (!is.null(comparison_group_plots_status$total_claims_closed_without_payment_status)){
                plot_content <- append(plot_content, list(comparison_group_total_claims_closed_without_payment_ggplot_export()))
                table_content <- append(table_content, list(comparison_group_total_claims_closed_without_payment_export_table()))
            }

            if (!is.null(comparison_group_plots_status$first_party_claims_closed_without_payment_status)){
                plot_content <- append(plot_content, list(comparison_group_first_party_claims_closed_without_payment_ggplot_export()))
                table_content <- append(table_content, list(comparison_group_first_party_claims_closed_without_payment_export_table()))
            }

            if (!is.null(comparison_group_plots_status$third_party_claims_closed_without_payment_status)){
                plot_content <- append(plot_content, list(comparison_group_third_party_claims_closed_without_payment_ggplot_export()))
                table_content <- append(table_content, list(comparison_group_third_party_claims_closed_without_payment_export_table()))
            }

            if (!is.null(comparison_group_plots_status$first_party_open_claims_status)){
                plot_content <- append(plot_content, list(comparison_group_first_party_open_claims_ggplot_export()))
                table_content <- append(table_content, list(comparison_group_first_party_open_claims_export_table()))
            }

            if (!is.null(comparison_group_plots_status$third_party_open_claims_status)){
                plot_content <- append(plot_content, list(comparison_group_first_party_open_claims_ggplot_export()))
                table_content <- append(table_content, list(comparison_group_third_party_open_claims_export_table()))
            }
            ...
            first_page_heights = switch(as.character(n_selected_group),
            ...
            content_page_heights = switch(as.character(n_selected_group),
...
output$export_comparison_group_excel = downloadHandler(
    filename = function() {glue("comparison_group.xlsx")},
    content = function(file) {
        withProgress(message = glue("Exporting comparison_group.xlsx"), {
            ...
            if (!is.null(comparison_group_plots_status$premium_written_status)){
                ...
                worksheet_content <- append(worksheet_content, list(comparison_group_written_premium_export_table_excel()))
            }

            if (!is.null(comparison_group_plots_status$yoy_change_premium_written_status)){
                worksheet_name <- append(worksheet_name, "YOY % Change")
                worksheet_content <- append(worksheet_content, list(comparison_group_yoy_change_written_premium_export_table_excel()))
            }

            if (!is.null(comparison_group_plots_status$premium_earned_status)){
                worksheet_name <- append(worksheet_name, "Earned Premiums")
                worksheet_content <- append(worksheet_content, list(comparison_group_earned_premium_export_table_excel()))
            }

            if (!is.null(comparison_group_plots_status$total_losses_status)){
                worksheet_name <- append(worksheet_name, "Direct Losses Paid")
                worksheet_content <- append(worksheet_content, list(comparison_group_total_losses_export_table_excel()))
            }

            if (!is.null(comparison_group_plots_status$loss_ratio_status)){
                worksheet_name <- append(worksheet_name, "Loss Ratio")
                worksheet_content <- append(worksheet_content, list(comparison_group_loss_ratio_export_table_excel()))
            }

            if (!is.null(comparison_group_plots_status$num_of_policies_status)){
                worksheet_name <- append(worksheet_name, "Num of Policies")
                worksheet_content <- append(worksheet_content, list(comparison_group_num_of_policies_export_table_excel()))
            }

            if (!is.null(comparison_group_plots_status$avg_premium_status)){
                worksheet_name <- append(worksheet_name, "Avg Premiums")
                worksheet_content <- append(worksheet_content, list(comparison_group_avg_premium_export_table_excel()))
            }

            if (!is.null(comparison_group_plots_status$direct_defense_status)){
                worksheet_name <- append(worksheet_name, "Direct Defense Paid")
                worksheet_content <- append(worksheet_content, list(comparison_group_direct_defense_export_table_excel()))
            }

            if (!is.null(comparison_group_plots_status$case_reserves_status)){
                worksheet_name <- append(worksheet_name, "Case Reserves")
                worksheet_content <- append(worksheet_content, list(comparison_group_case_reserves_export_table_excel()))
            }

            if (!is.null(comparison_group_plots_status$total_claims_closed_with_payment_status)){
                worksheet_name <- append(worksheet_name, "With Payment")
                worksheet_content <- append(worksheet_content, list(comparison_group_total_claims_closed_with_payment_export_table_excel()))
            }

            if (!is.null(comparison_group_plots_status$total_claims_closed_without_payment_status)){
                worksheet_name <- append(worksheet_name, "Without Payment")
                worksheet_content <- append(worksheet_content, list(comparison_group_total_claims_closed_without_payment_export_table_excel()))
            }

            if (!is.null(comparison_group_plots_status$first_party_claims_closed_without_payment_status)){
                worksheet_name <- append(worksheet_name, "1st Without Payment")
                worksheet_content <- append(worksheet_content, list(comparison_group_first_party_claims_closed_without_payment_export_table_excel()))
            }

            if (!is.null(comparison_group_plots_status$third_party_claims_closed_without_payment_status)){
                worksheet_name <- append(worksheet_name, "3rd Without Payment")
                worksheet_content <- append(worksheet_content, list(comparison_group_third_party_claims_closed_without_payment_export_table_excel()))
            }

            if (!is.null(comparison_group_plots_status$first_party_open_claims_status)){
                worksheet_name <- append(worksheet_name, "1st Open Claims")
                worksheet_content <- append(worksheet_content, list(comparison_group_first_party_open_claims_export_table_excel()))
            }

            if (!is.null(comparison_group_plots_status$third_party_open_claims_status)){
                worksheet_name <- append(worksheet_name, "3rd Open Claims")
                worksheet_content <- append(worksheet_content, list(comparison_group_third_party_open_claims_export_table_excel()))
            }
